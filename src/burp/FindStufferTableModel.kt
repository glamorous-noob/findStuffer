...
class FindStufferTableModel : AbstractTableModel() {
    ...
    private val columns = arrayOf(
        ...
        FindStufferColumn("Protocol" ,  { row -> row.protocol }, compareBy<FindStufferRowData> { it.protocol }.thenBy { it.number }),
        FindStufferColumn("Host" , { row -> row.url.host }, compareBy<FindStufferRowData> { it.url.host}.thenBy { it.number }),
        FindStufferColumn("URL Path" , { row -> row.url.path }, compareBy<FindStufferRowData> { it.url.path}.thenBy { it.number }),
        FindStufferColumn("Method" , { row -> row.method }, compareBy<FindStufferRowData> { it.method}.thenBy { it.number }),
        FindStufferColumn("Status" , { row -> row.statusCode ?: "" }, compareBy<FindStufferRowData> { it.statusCode}.thenBy { it.number }),
        FindStufferColumn("Response Length" , { row -> row.responseLength ?: "" }, compareBy<FindStufferRowData> { it.responseLength}.thenBy { it.number }),
        FindStufferColumn("MIME Type" , { row -> row.mimeType ?: ""  }, compareBy<FindStufferRowData> { it.mimeType}.thenBy { it.number })
    )

    // The non-sorted non-filtered version, the underlying list
    var allRows : List<FindStufferRowData> = arrayListOf()
    ...
    var filteredSortedRows : MutableList<FindStufferRowData> = arrayListOf()
    ...
    private var appliedFilter : IFindStufferRowFilter? = null
    ...
    fun initContent(data : Array<IHttpRequestResponse>){
        allRows = data.mapIndexed { index, element -> FindStufferRowData(index+1, element) }
    ...
    private fun getColumnComparator(column : Int) : Comparator<FindStufferRowData> {
    ...
    fun useNewFilter(filter: IFindStufferRowFilter) {
    ...
    private class FindStufferColumn(val name : String, val cellDisplayedValGetter : Function<FindStufferRowData, Any>, val comparator: Comparator<FindStufferRowData>){
